...
import { ShieldAlert, AlertTriangle, CheckCircle2, Lightbulb, SearchX, FileText, Clock } from 'lucide-react'
import type { AuditDeliverable } from './page'
...
interface AuditClientProps {
  ...
  auditDeliverables: AuditDeliverable[]
}
...
export default function AuditClient({
  ...
  return (
    <div className="space-y-8">
      ...
      {auditDeliverables.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-4">
            <FileText className="size-5 text-torii" />
            <h2 className="font-serif text-lg font-medium text-charcoal">
              Audit Reports from Deliverables
            </h2>
          </div>
          <p className="text-sm text-warm-gray mb-4">
            Additional audit reports generated by your Tenkai team
          </p>
          ...
          <div className="space-y-3">
            {auditDeliverables.map((d) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-medium text-charcoal">{d.title ?? 'Untitled Report'}</span>
                    <span className="rounded-full bg-parchment px-2 py-0.5 text-[10px] font-medium text-charcoal">
                      {(d.deliverable_type ?? 'report').replace(/_/g, ' ')}
                    </span>
                    {d.score != null && (
